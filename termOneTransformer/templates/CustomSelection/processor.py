...
def create_mlo(input_json_data, input_other_jsons_data, exiting_hashcode):
    ...
    try:
        text_en_data = input_json_data['templateConfig'][0]['title']

    except Exception as e:
        msg = f'Error: {input_json_data["templateConfig"][0]["title"]} --> {e}'
        STATUS.append(msg)
        raise Exception(msg)
    ...
    try:
        thumbs = input_json_data["templateConfig"][0]['cards']
    ...
    for each_thumb in thumbs:
        ...
        try:
            image_thumb_relative_path = each_thumb['image']
        except Exception as e:
            msg = f'Error: {input_json_data["pageData"]["templateID"]} --> {e}'
            STATUS.append(msg)
            raise Exception(msg)
    ...
    from termOneTransformer.helpers import get_xml_feedback, get_xml_hint
    try:
        feedback = {}
        feedback["correct"] = input_json_data["templateConfig"][0].get("correctPopUp", None)
        feedback["incorrect_1"] = input_json_data["templateConfig"][0].get("incorrectPopUp", None)
        feedback["incorrect_2"] = input_json_data["templateConfig"][0].get("incorrectPopUp2", None)
        # get feedback xml
    ...
    try:
        hint = input_json_data["templateConfig"][0].get("hintText", None)
        # get feedback xml
    ...
    try:
        ...
        src_audio_path = os.path.join(input_json_data['graphics_path'] + "/audio/audio.mp3")
