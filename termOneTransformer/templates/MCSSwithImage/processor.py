from termOneTransformer.helpers import (generate_unique_folder_name, convert_html_to_strong, get_teacher_note,
...
def create_mlo(input_json_data, input_other_jsons_data, exiting_hashcode):
    ...
    try:
        ques = input_json_data['templateConfig'][0]['title']
    except:
    ...
    try:
        src = os.path.join(input_json_data['graphics_path']+'/audio/audio.mp3')
    except:
        raise Exception("Error: MCSS_001 --> src not found")

    image_check = input_json_data['templateConfig'][0].get('image', None)
    ...
    options = input_json_data['templateConfig'][0].get('options', None)
    ...
    for each_op in options:
        ...
        is_correct = "Yes" if each_op['answer'] == True else "No"
    ...
    try:
        feedback = {}
        feedback["correct"] = input_json_data["templateConfig"][0]["messages"].get('correct', None)
        feedback["incorrect_1"] = input_json_data["templateConfig"][0]["messages"].get('incorrect1', None)
        feedback["incorrect_2"] = input_json_data["templateConfig"][0]["messages"].get('incorrect2', None)
        # get feedback xml
    ...
    try:
        hint = input_json_data["templateConfig"][0].get("hint", None)
        # get feedback xml
