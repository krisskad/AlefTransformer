...
def get_input_dir_obj(INPUT_DIR, output_dir, INPUT_COMMON_DIR):
    ...
    for each_course_dir in all_courses_dir:
        ...
        INPUT_APP_DIR = os.path.join(INPUT_DIR, course_dir)
        INPUT_STRUCTURE_JSON = os.path.join(INPUT_APP_DIR, "data.json")
        paths_exist = validate_paths(INPUT_STRUCTURE_JSON)

        if paths_exist is False:
            print("Path does not exist")
            break

        path_obj = { "INPUT_STRUCTURE_JSON": INPUT_STRUCTURE_JSON,  "COURSE_ID": course_dir, "INPUT_COMMON_DIR": INPUT_COMMON_DIR,
            "OUTPUT_DIR":output_dir,
                        }
        all_input_objects.append(path_obj)
